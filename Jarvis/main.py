...
# formatting the output sentence in good and compatible format
...
# Function speaking out the audio produced
def speak():
    pygame.init()
...
# audio produced after giving command
def yesmaster():
    pygame.init()
...
# audio produced when our project Come live
...
# Audio is produced when The AI is unable to understand the command or there is any noise
...
# First beep after which user say command
def beep1():
    pygame.init()
...
# Second beep jarvis completed its task
def beep2():
    pygame.init()
...
# Sound is generated by this function
...
* For filling Water: Motor Pump
       ...
       # Iterating various equipments for switch on or off     
...
def main():
    time.sleep(10)
    jarviscametolive()
    # code below is For wake up call
    porcupine=None
    pa=None
    audio_stream=None
